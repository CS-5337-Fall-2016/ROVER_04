...
public class ROVER_04 {
	...
	int centerIndex = (scanMap.getEdgeSize() - 1)/2;
	MapTile tile = scanMapTiles1[centerIndex][centerIndex];
    Science science = tile.getScience();
	if(science.equals(Science.RADIOACTIVE))
		System.out.println("ROVER_04 is requesting GATHER organic!");
		out.println("GATHER");
    Thread.sleep(200);
	int m,n=0;
    Object currentDirection;if((scanMapTiles[centerIndex-1][centerIndex].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex-1][centerIndex].getTerrain()==Terrain.ROCK)
    		||(scanMapTiles[centerIndex-2][centerIndex].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex-2][centerIndex].getTerrain()==Terrain.ROCK&&
			!isBlocked(scanMapTiles[centerIndex-1][centerIndex]))){
		currentDirection = 3;
		basicMove(currentDirection, scanMapTiles, centerIndex);
	}
	if((scanMapTiles[centerIndex+1][centerIndex].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex+1][centerIndex].getTerrain()==Terrain.ROCK)
			||(scanMapTiles[centerIndex+2][centerIndex].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex+2][centerIndex].getTerrain()==Terrain.ROCK&&
			!isBlocked(scanMapTiles[centerIndex+1][centerIndex]))){
		currentDirection = 1;
		basicMove(currentDirection, scanMapTiles, centerIndex);
	}
	if((scanMapTiles[centerIndex][centerIndex-1].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex][centerIndex-1].getTerrain()==Terrain.ROCK)
			||(scanMapTiles[centerIndex][centerIndex-2].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex][centerIndex-2].getTerrain()==Terrain.ROCK&&
	...
	if((scanMapTiles[centerIndex][centerIndex+1].getScience()==Science.RADIOACTIVE&&scanMapTiles[centerIndex][centerIndex+1].getTerrain()==Terrain.ROCK)
		...
		basicMove(currentDirection, scanMapTiles, centerIndex);
    		...
    		private void basicMove(Object currentDirection, MapTile[][] scanMapTiles, int centerIndex) {
		// TODO Auto-generated method stub
